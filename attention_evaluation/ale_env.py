...
def _canonize(game: str) -> str:
    # 'freeway', 'FreewayNoFrameskip-v4', 'ALE/Freeway-v5' -> 'freeway'
    g = game.split("/")[-1]
    for suf in ("NoFrameskip-v4", "-v4", "-v5"):
        g = g.replace(suf, "")
...
def _get_rom(game):
    ...
    # Preferred path on ale-py >= 0.9
    try:
        return roms.get_rom_path(name)
    except Exception:
        pass  # fall through to env var lookup

    # Fallback: user-installed ROM directory (AutoROM/manual)
    for var in ("ALE_ROM_DIR", "ALE_PY_ROM_DIR"):
        root = os.environ.get(var)
        if root:
            for cand in (f"{name}.bin", f"{name}.zip"):
                p = os.path.join(root, cand)
                if os.path.exists(p):
                    return p

    raise RuntimeError(
        f"Could not locate ROM for '{game}'. "
        "Install ROMs with AutoROM or set ALE_ROM_DIR to the folder containing e.g. 'freeway.bin'."
    )
