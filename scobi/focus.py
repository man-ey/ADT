...
class Focus():
    ...
    #local
    def get_feature_vector_local(self, obs):

        from collections import defaultdict

        # --- 1) Exactly like your original code: build the property layer ---
        assert obs.shape[0] == 2, "OC_Atari window-buffer size should be 2"
        # Convert the two-frame observation into a 1D NumPy array with position history, etc.
        inc_ns_repr_array = self.add_history_to_obs(obs)
        # Turn that array into a list of property tuples, one entry per NS_REPR_TYPES slot
        self.CURRENT_PROPERTY_COMPUTE_LAYER = self.ns_repr_list_to_func_input(inc_ns_repr_array)

        # ---------------------------------------------------------------------
        # 2) Identify which tuples belong to which object, and find agent's (x,y)
        # ---------------------------------------------------------------------
        #
        # Recall: self.NS_REPR_LIST is parallel to self.NS_REPR_TYPES.
        # For i in range(len(self.NS_REPR_LIST)):
        #   - self.NS_REPR_LIST[i] might be ("POSITION", "Enemy3") or ("POSITION_HISTORY", "Player1"), etc.
        #   - self.CURRENT_PROPERTY_COMPUTE_LAYER[i] is the actual tuple of data for that property+object.
        #
        # We'll group them by object_name, so we can find each object's position and zero it out if needed.

        object_map = defaultdict(dict)   # object_map[name]["POSITION"] = (x,y)
        # object_map[name]["INDEXES"] = [all i that belong to name]
        all_object_names = set()
        for i, (meaning, obj_name) in enumerate(self.NS_REPR_LIST):
            all_object_names.add(obj_name)
            if "INDEXES" not in object_map[obj_name]:
                object_map[obj_name]["INDEXES"] = []
            object_map[obj_name]["INDEXES"].append(i)

            # If this slot is a POSITION, store the (x,y) so we can compute distance
            if meaning == "POSITION":
                # self.CURRENT_PROPERTY_COMPUTE_LAYER[i] might be (x, y)
                object_map[obj_name]["POSITION"] = self.CURRENT_PROPERTY_COMPUTE_LAYER[i]

        # Identify the "agent" by picking any object that definitely has a "POSITION"
        # If you specifically know your agent is named "Player1," you can do:
        #   agent_name = "Player1"
        # Or automatically pick the first in dictionary order that has a position:
        agent_name = None
        #for name in object_map:
        #   if "POSITION" in object_map[name]:
        #      agent_name = name
        #     break
        if agent_name is None:
            # If for some reason no agent found, just skip (no distance logic).
            # We'll proceed with the original code and do no truncation.
            pass
        else:
            agent_pos = object_map[agent_name]["POSITION"]
            ax, ay = agent_pos[0], agent_pos[1]

            # Compute distances for all other objects that have a POSITION
            ...
            for name, info in object_map.items():
                ...
                if "POSITION" in info:
                    ...
                    dist_list.append((dist_sq, name))

            # Sort by distance ascending, keep up to 5
            dist_list.sort(key=lambda x: x[0])
            keep_names = set([item[1] for item in dist_list[:5]])
            keep_names.add(agent_name)  # always keep the agent
            #SPECIAL_ALWAYS_KEEP = {"Ball1"}
            #keep_names.update(n for n in SPECIAL_ALWAYS_KEEP if n in object_map)
            # -----------------------------------------
            # 3) Zero out every object not in keep_names
            # -----------------------------------------
            # For each object that we are *not* keeping, set all its property data to 0
            for name in object_map:
                if name not in keep_names:
                    # For each slot in CURRENT_PROPERTY_COMPUTE_LAYER that belongs to this object,
                    # we set it to a tuple of zeros with the same length as before.
                    for idx in object_map[name]["INDEXES"]:
                        old_tuple = self.CURRENT_PROPERTY_COMPUTE_LAYER[idx]
                        zeroed = tuple(0 for _ in old_tuple)
                        self.CURRENT_PROPERTY_COMPUTE_LAYER[idx] = zeroed

        # ------------------------------------------------------
        # 4) Now we do the usual function-layer computation etc.
        # ------------------------------------------------------
        for i in range(self.FUNC_COMPUTE_LAYER_SIZE):
            f = self.FUNC_COMPUTE_LAYER[i]
            self.CURRENT_FUNC_COMPUTE_LAYER[i] = f(self.CURRENT_PROPERTY_COMPUTE_LAYER)

        # The rest is exactly like your original get_feature_vector code:

        if self.first_pass:
            self.first_pass = False
            props = [val for row in self.CURRENT_PROPERTY_COMPUTE_LAYER for val in row]
            funcs = [val for row in self.CURRENT_FUNC_COMPUTE_LAYER for val in row]
            out = props + funcs
            ...
            final_out = self.last_obs_vector
            if self.HIDE_PROPERTIES:
                final_out = final_out[self.FEATURE_VECTOR_PROPS_SIZE:]
            return np.asarray(final_out, dtype=np.float32), reward
        ...
        self.last_obs_vector = out
        reward = self.REWARD_FUNC(out) if self.REWARD_SHAPING != 0 else 0
        final_out = out
        if self.HIDE_PROPERTIES:
            final_out = final_out[self.FEATURE_VECTOR_PROPS_SIZE:]
        return np.asarray(final_out, dtype=np.float32), reward
